...
const tools = [
    {
        ...
        description: "Criar uma nova sessão de estudo com respostas. Retorna session_id e XP ganho.",
        inputSchema: {
            type: "object",
            properties: {
                user_id: {
    ...
    try {
        switch (name) {
            case "create_study_session": {
                const { user_id, topic_id, platform_id, total_questions, correct_answers, answers, source = "baked", } = args;
                const wrong_answers = total_questions - correct_answers;
                const accuracy_pct = total_questions > 0
                    ? Math.round((correct_answers / total_questions) * 100 * 100) / 100
                    : 0;
                const is_perfect = total_questions > 0 &&
            ...
            case "update_user_topic_progress": {
                const { user_id, topic_id, status } = args;
            ...
            case "get_user_gamification": {
                const { user_id, platform_id } = args;
            ...
            case "list_topics": {
                ...
                if (exam_id && !section_id) {
                    ...
                    filtered = (data || []).filter((t) => sectionIds.has(t.section_id));
                }
                return {
                    content: [
                        {
            ...
            case "award_achievement": {
                const { user_id, achievement_slug, session_id } = args;
            ...
            case "get_leaderboard": {
                const { platform_id, limit = 10 } = args;
                const { data, error } = await supabase
